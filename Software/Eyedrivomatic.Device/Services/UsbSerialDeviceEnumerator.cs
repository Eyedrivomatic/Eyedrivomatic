...
namespace Eyedrivomatic.Device.Services
{
    ...
    {
        public static readonly Dictionary<string, DeviceIdFilter> UnoDeviceIds =
            new Dictionary<string, DeviceIdFilter>
            {
                {"Arduino Uno R3", new DeviceIdFilter("2341", "0001", "0043", "0243")},
                {"Genuino Uno R3", new DeviceIdFilter("2A03", "0001", "0043", "0243")}
            };

        public static readonly Dictionary<string, DeviceIdFilter> TeencyDeviceIds =
            new Dictionary<string, DeviceIdFilter>
            {
                {"Teencyduino Serial", new DeviceIdFilter("16C0", "0483")}
            };
    }
    ...
    {
        public static IEnumerable<UsbSerialDevice> EnumerateDevices([AllowNull] IList<DeviceIdFilter> filter = null)
        {
            ...
            try
            {
                ...
                for (uint i = 0; NativeMethods.SetupDiEnumDeviceInfo(hDevInfoSet, i, ref devInfoData); i++)
                {
                    ...
                    {
                        ConnectionString = GetPortName(hDevInfoSet, devInfoData),
                    ...
                    if (filter != null && !filter.Where(f => f.Vid == device.Vid).SelectMany(f => f.Pids).Contains(device.Pid)) continue;
