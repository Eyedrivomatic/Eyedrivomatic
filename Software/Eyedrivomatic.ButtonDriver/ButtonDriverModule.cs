            ...
            try
            {
                ...
                if (_configurationService.AutoConnect)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(connectionString))
                    {
                        ...
                        await _hardwareService.CurrentDriver.ConnectAsync(connectionString, true, CancellationToken.None);
                    }
                    else
                    {
                        Log.Warn(this, "Connection string not specified. Attempting to auto-detect.");
                        await _hardwareService.CurrentDriver.AutoConnectAsync(false, CancellationToken.None);
